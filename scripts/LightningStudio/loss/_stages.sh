...
#   bash scripts/LightningStudio/run.sh loss/l2_all.sh STAGE=fit GAP_R=3
...
SEARCH_GPUS="${SEARCH_GPUS:-1}"       # parallel workers sharing the sqlite study
MIX_MIN="${MIX_MIN:-0.2}"
MIX_MAX="${MIX_MAX:-0.8}"
...
RUN_DIR="${INSTAROAD_ROOT}/runs/loss_${FULL_TAG}_seed${SEED}"
...
if [ "$STAGE" = "bench" ]; then
  ...
  echo "          python scripts/phase_a_report.py --runs ${INSTAROAD_ROOT}/runs"
  exit 0
fi
...
echo "Next: re-run this arm with STAGE=bench (or use run_both.sh to chain fit->bench)."
