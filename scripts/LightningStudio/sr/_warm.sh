...
if [ ! -f "$WARM_START_CKPT" ]; then
  ...
  echo "  Run first:  bash scripts/LightningStudio/run.sh sr/${STAGE1_TAG}.sh STAGE=tune SEED=${SEED}${LOSS_ARM:+ LOSS_ARM=${LOSS_ARM}}" >&2
  echo "  then:       bash scripts/LightningStudio/run.sh sr/${STAGE1_TAG}.sh STAGE=fit  SEED=${SEED}${LOSS_ARM:+ LOSS_ARM=${LOSS_ARM}}" >&2
  exit 1
fi
