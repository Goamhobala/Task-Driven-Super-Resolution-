#!/bin/bash
# SEED REFIT — sr_r0_new_gap_t2_ce_dice_holdout
#
...
#
#   cd scripts/hpc
#   sbatch --job-name=refit-gap_t2_ce_dice --time=12:00:00 \
#          --gres=gpu:1 --cpus-per-task=8 \
#          train.sbatch --SCRIPT=loss/refit/gap_t2_ce_dice.sh
#
...
#   sr_r0_new_gap_t2_ce_dice_holdout_seed0
...
LOSS_ARM="pstar_dice"
export MODEL_NAME="sr_r0_new_gap_t2_ce_dice_holdout"   # match seed 0 so the store groups the seeds
SEEDS="${SEEDS:-1 2}"
...
model:
  ...
  freeze_sr: false
  sr_pad: 0
  lr: 0.0003392292260979527
  loss_arm: pstar_dice
  pstar: gap_t2_ce
  gap_r: 4
  gap_k: 60.0
  tl_ell: 5
  tl_theta: 0.4794392197737672
  gap_theta: 0.5297063519481312
  tversky_alpha: 0.7
  cl_alpha: 0.3
  cl_iters: 5
  sr_w: 1.0
  ...
  warmup_start: 15
  warmup_ramp: 5
  mix_w: 0.476736052208817
  pos_weight: 2.1248222047017635
  lr_schedule: cosine
...
echo "=== sr_r0_new_gap_t2_ce_dice_holdout: seeds ${SEEDS} done ==="
