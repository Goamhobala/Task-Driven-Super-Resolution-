#!/bin/bash
# SEED REFIT — sr_r0_new_gap_t4_ce_holdout
#
...
#
#   cd scripts/hpc
#   sbatch --job-name=refit-gap_t4_ce --time=12:00:00 \
#          --gres=gpu:1 --cpus-per-task=8 \
#          train.sbatch --SCRIPT=loss/refit/gap_t4_ce.sh
#
...
#   sr_r0_new_gap_t4_ce_holdout_seed0_L4_modal
...
EXP_TAG="r0_new"
LOSS_ARM="gap_t4_ce"
export MODEL_NAME="sr_r0_new_gap_t4_ce_holdout"   # match seed 0 so the store groups the seeds
SEEDS="${SEEDS:-1 2}"
...
for SEED in $SEEDS; do
  ...
  if [ -f "$RUN_DIR/checkpoints/unet_s2rosa_jointsr_final.ckpt" ] \
    ...
    echo "########## sr_r0_new_gap_t4_ce_holdout  SEED=${SEED}  FIT already done — skipping ##########"
  else
    echo "########## sr_r0_new_gap_t4_ce_holdout  SEED=${SEED}  FIT ##########"
  ...
  if in_store "$SEED" val; then
    echo "########## sr_r0_new_gap_t4_ce_holdout  SEED=${SEED}  BENCH val already in store — skipping ##########"
  else
    echo "########## sr_r0_new_gap_t4_ce_holdout  SEED=${SEED}  BENCH val (selects theta*) ##########"
  ...
  if [ "${RUN_TEST:-1}" = "1" ]; then
    if in_store "$SEED" test; then
      echo "########## sr_r0_new_gap_t4_ce_holdout  SEED=${SEED}  BENCH test already in store — skipping ##########"
    else
      echo "########## sr_r0_new_gap_t4_ce_holdout  SEED=${SEED}  BENCH test (at val's theta*) ##########"
      env EXP_TAG="$EXP_TAG" LOSS_ARM="$LOSS_ARM" SEED="$SEED" STAGE=bench \
...
echo "=== sr_r0_new_gap_t4_ce_holdout: seeds ${SEEDS} done ==="
