#!/bin/bash
# SEED REFIT — sr_r0_new_gap_tl_ce_holdout
#
...
#
#   cd scripts/hpc
#   sbatch --job-name=refit-gap_tl_ce --time=12:00:00 \
#          --gres=gpu:1 --cpus-per-task=8 \
#          train.sbatch --SCRIPT=loss/refit/gap_tl_ce.sh
#
...
#   sr_r0_new_gap_tl_ce_holdout_seed0_L4_lightning
...
EXP_TAG="r0_new"
LOSS_ARM="gap_tl_ce"
export MODEL_NAME="sr_r0_new_gap_tl_ce_holdout"   # match seed 0 so the store groups the seeds
SEEDS="${SEEDS:-1 2}"
...
model:
  ...
  freeze_sr: false
  sr_pad: 0
  lr: 0.0004392670615201874
  loss_arm: gap_tl_ce
  pstar: bce
  gap_r: 4
  gap_k: 60.0
  tl_ell: 5
  tl_theta: 0.5409065645350193
  gap_theta: 0.38105240274638613
  tversky_alpha: 0.7
  cl_alpha: 0.3
  cl_iters: 5
  sr_w: 1.0
...
for SEED in $SEEDS; do
  ...
  if [ -f "$RUN_DIR/checkpoints/unet_s2rosa_jointsr_final.ckpt" ] \
    ...
    echo "########## sr_r0_new_gap_tl_ce_holdout  SEED=${SEED}  FIT already done — skipping ##########"
  else
    echo "########## sr_r0_new_gap_tl_ce_holdout  SEED=${SEED}  FIT ##########"
  ...
  if in_store "$SEED" val; then
    echo "########## sr_r0_new_gap_tl_ce_holdout  SEED=${SEED}  BENCH val already in store — skipping ##########"
  else
    echo "########## sr_r0_new_gap_tl_ce_holdout  SEED=${SEED}  BENCH val (selects theta*) ##########"
  ...
  if [ "${RUN_TEST:-1}" = "1" ]; then
    if in_store "$SEED" test; then
      echo "########## sr_r0_new_gap_tl_ce_holdout  SEED=${SEED}  BENCH test already in store — skipping ##########"
    else
      echo "########## sr_r0_new_gap_tl_ce_holdout  SEED=${SEED}  BENCH test (at val's theta*) ##########"
      env EXP_TAG="$EXP_TAG" LOSS_ARM="$LOSS_ARM" SEED="$SEED" STAGE=bench \
...
echo "=== sr_r0_new_gap_tl_ce_holdout: seeds ${SEEDS} done ==="
