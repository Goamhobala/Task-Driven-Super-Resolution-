...
for SEED in $SEEDS; do
  ...
  if [ -f "$RUN_DIR/checkpoints/unet_s2rosa_jointsr_final.ckpt" ] \
    ...
    echo "########## sr_r0_new_lcdice_holdout  SEED=${SEED}  FIT already done — skipping ##########"
  else
  ...
  if in_store "$SEED" val; then
    echo "########## sr_r0_new_lcdice_holdout  SEED=${SEED}  BENCH val already in store — skipping ##########"
  else
  ...
  if [ "${RUN_TEST:-1}" = "1" ]; then
    if in_store "$SEED" test; then
      echo "########## sr_r0_new_lcdice_holdout  SEED=${SEED}  BENCH test already in store — skipping ##########"
    else
