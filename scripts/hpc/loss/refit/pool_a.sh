...
#
#   cd scripts/hpc
#   sbatch --job-name=refit-pool-a --time=48:00:00 \
#          --gres=gpu:2 --ntasks=2 --cpus-per-task=8 \
#          train.sbatch --SCRIPT=loss/refit/pool_a.sh
#
# --ntasks=2 --cpus-per-task=8 requests 16 cores while staying inside an
...
# rather than 16, the site binds the batch step to one task's cores and the
