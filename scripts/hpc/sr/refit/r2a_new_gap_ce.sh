#!/bin/bash
# SEED REFIT — sr_r2a_new_gap_ce_anorm_recalpost
#
# R2a — COLD joint task-driven SEN2SR fine-tuning WITH the FFT hard-constraint
# bundle and 8 px padding. Same loss arm as R0, so R2a minus R0 isolates the
# SR front-end.
#
...
#   SEEDS="3 4" sbatch ... train.sbatch --SCRIPT=sr/refit/r2a_new_gap_ce.sh
#
...
#
# Hyperparameters are seed 66's tune (best val_ap=0.5950), copied verbatim from
#   sr_r2a_new_gap_ce_anorm_recalpost_seed66
...
RUN_TAG="sr_r2a_new_gap_ce_anorm_recalpost"
export MODEL_NAME="sr_r2a_new_gap_ce_anorm_recalpost_ap"   # matches the tuned seed so the store groups them
