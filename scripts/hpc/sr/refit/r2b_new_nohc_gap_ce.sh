#!/bin/bash
# SEED REFIT — sr_r2b_new_nohc_gap_ce_anorm_recalpost
#
# R2b — COLD joint SEN2SR-Lite fine-tuning with the FFT HARD CONSTRAINT OFF
# (bare generator: no positivity clamp, no frequency splice, no pad). SR_HC=off
# puts _nohc into the run dir, the study and the bench model_name.
#
# Same loss arm (gap_ce) and same tuned seed (66) as r0_new_gap_ce and
# r2a_new_gap_ce, so the three form a controlled column: identical task loss,
# identical protocol, ONLY the upsampler and its constraint differ.
#   r0  bicubic                          r2a  SEN2SR + HC bundle, pad 8
#                                        r2b  SEN2SR bare, no HC, pad 0
#
...
#
#   cd scripts/hpc
#   sbatch --job-name=refit-r2b_new_nohc_gap_ce --time=24:00:00 \
#          --gres=gpu:1 --cpus-per-task=8 \
#          train.sbatch --SCRIPT=sr/refit/r2b_new_nohc_gap_ce.sh
#
# CHANGE THE SEEDS AND NOTHING ELSE:
#   sbatch ... train.sbatch --SCRIPT=sr/refit/r2b_new_nohc_gap_ce.sh   # SEEDS defaults below
#   SEEDS="3 4" sbatch ... train.sbatch --SCRIPT=sr/refit/r2b_new_nohc_gap_ce.sh
#
...
#
# Hyperparameters are seed 66's tune (best val_ap=0.5892), copied verbatim from
#   sr_r2b_new_nohc_gap_ce_anorm_recalpost_seed66
...
RUN_TAG="sr_r2b_new_nohc_gap_ce_anorm_recalpost"
export MODEL_NAME="sr_r2b_new_nohc_gap_ce_anorm_recalpost_ap"   # matches the tuned seed
