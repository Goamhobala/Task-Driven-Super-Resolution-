...
# Sourced by rl4.sh only on this platform (rl2 runs in the Studio), AFTER
# _rl_common.sh and BEFORE the engine. Not run directly.
#
# CLUSTER TWIN of scripts/LightningStudio/sr/rl/_rl_rung.sh — byte-identical
# below this header, and pinned so by tests/test_rl_campaign_hpc.py. A rung that
# means one thing in the Studio and another here would put two different doses
# under one _ls tag.
#
