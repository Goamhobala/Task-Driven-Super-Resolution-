...
SEARCH_GPUS="${SEARCH_GPUS:-2}"    # one tuner process per GPU; match the allocation
...
if [ "$STAGE" = "tune" ]; then
  ...
  echo "Next: sbatch --gres=gpu:1 scripts/hpc/train.sbatch --SCRIPT=unet/${EXP_TAG}.sh STAGE=fit SEED=${SEED}"
  exit 0
fi
