...
def _draw(ax_i, ax_m, ax_o, ds, idx, mode, rgb, pct):
    ...
    ax_o.clear()
    ax_i.imshow(rgb_img)
    ax_i.set_title("enhanced RGB" if mode == "enhanced" else "RGB satellite")
    ax_i.axis("off")
...
def preview_dataloader(dataset_dir, split="train", mode="normal",
                       image_size=256, rgb=(0, 1, 2), pct=(2, 98), start=0):
    """Interactive viewer over ``split``: RGB crop | ground-truth mask | red overlay.
    ...
    ds = _build_dataset(dataset_dir, split, RGB_MODES[mode], image_size)
    n = len(ds)
    if n == 0:
    ...
    def show():
        name = _draw(ax_i, ax_m, ax_o, ds, state["idx"], mode, rgb, pct)
...
def save_previews(dataset_dir, split="train", out_dir="previews", n=8,
                  mode="normal", image_size=256, rgb=(0, 1, 2), pct=(2, 98),
                  start=0):
    ...
    for i in range(count):
        idx = (start + i) % len(ds)
        fig, (ax_i, ax_m, ax_o) = plt.subplots(1, 3, figsize=(13.5, 4.8))
        name = _draw(ax_i, ax_m, ax_o, ds, idx, mode, rgb, pct)
...
if __name__ == "__main__":
    ...
    if args.save:
        save_previews(args.dataset_dir, split=args.split, out_dir=args.save,
                      n=args.n, mode=args.mode, image_size=args.image_size, start=args.start)
    else:
        preview_dataloader(args.dataset_dir, split=args.split, mode=args.mode,
