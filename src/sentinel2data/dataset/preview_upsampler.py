...
def _draw(ax_i, ax_m, ds, idx, upscale, rgb, pct):
    """Render item ``idx`` (bicubic RGB | graph mask) into the two axes."""
    ...
    ax_m.clear()
    ax_i.imshow(rgb_img)
    ax_i.set_title(f"bicubic RGB  {edge}px (x{upscale})")
    ax_i.axis("off")
...
def preview_upsampler(dataset_dir, split="train", bands=DEFAULT_BANDS,
                      crop_size=128, upscale=4, rgb=(0, 1, 2), pct=(2, 98), start=0):
    """Interactive viewer over ``split``: bicubic RGB crop | graph-rasterised mask.

    Arrow-key (or space) to step; ``q`` to quit. Returns the built dataset.
    """
    import matplotlib.pyplot as plt

    ds = _build_dataset(dataset_dir, split, bands, crop_size, upscale)
    n = len(ds)
    if n == 0:
    ...
    def show():
        name = _draw(ax_i, ax_m, ds, state["idx"], upscale, rgb, pct)
...
def save_previews(dataset_dir, split="train", out_dir="previews_upsampled", n=8,
                  bands=DEFAULT_BANDS, crop_size=128, upscale=4, rgb=(0, 1, 2),
                  pct=(2, 98), start=0):
    ...
    for i in range(count):
        idx = (start + i) % len(ds)
        fig, (ax_i, ax_m) = plt.subplots(1, 2, figsize=(9, 4.8))
        name = _draw(ax_i, ax_m, ds, idx, upscale, rgb, pct)
...
if __name__ == "__main__":
    ...
    if args.save:
        save_previews(args.dataset_dir, split=args.split, out_dir=args.save,
                      n=args.n, crop_size=args.crop_size, upscale=args.upscale,
                      start=args.start)
    else:
        preview_upsampler(args.dataset_dir, split=args.split,
