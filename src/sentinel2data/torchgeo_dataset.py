...
def build_dataset(dataset_dir, split="train", bands=RGB_BANDS, crs=WGS84,
    ...
    20-band; pass :class:`S2RosaV2Image` for the 23-band S2-ROSA-V2 imagery so the
    appended enhanced-RGB bands are addressable). ``crs`` is the common working CRS
    every tile is reprojected to (default EPSG:4326 / WGS84, so all UTM zones share
    one grid); ``crs=None`` keeps the native CRS of the first tile (no warp)."""
